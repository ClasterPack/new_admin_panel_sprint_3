...
class DataTransform:
    ...
    def transform_fw_data(self, data: List[dict]) -> List[dict] or None:
        bulk_data= []
        fw_objects =[
            self.type_adapter.validate_python(movie) for movie in data
        ]
        for movie in fw_objects:
            action = {
                ...
                "_source": {
                    ...
                    "directors": [{"id": director.id, "name": director.name}
                                  for director in movie.directors] if movie.directors else None,
                    "actors": [{'id': actor.id, "name": actor.name}
                               for actor in movie.actors] if movie.actors else None,
                    "writers": [{'id': writer.id, "name": writer.name}
                               for writer in movie.writers] if movie.writers else None,
                },
            }
    ...
    def transform_genres_updates(self, movies: List[dict]) -> List[dict] or None:
        bulk_data = []
        fw_objects = [
            self.type_adapter.validate_python(movie) for movie in movies
        ]
        for movie in fw_objects:
            action = {
                "_op_type": "update",
