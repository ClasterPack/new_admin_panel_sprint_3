...
class ElasticExtraction:
    def __init__(
            self, setup: Settings,
            state: State, data_transform: DataTransform,
    ):
        self.setup = setup
        self.state = state
    ...
    def transform_data_from_pg(self, model: Type[BaseModel], index_name , settings, state_key):
        ...
        if (transformed_movies_data
                and transformed_movies_data[-1]['_id'] == self.state.get_state(key=state_key)):
    ...
    def update_genre(self,state_key):
        while self.pg.get_updated_objects_ids('genre', state_key):
            genre_updates, last_update_genre = self.pg.get_updated_objects_ids('genre', state_key)
            updates_from_pg =self.pg.updates_genres(genre_updates)
            transformed_genre_data = self.data_transform.transform_genres_updates(updates_from_pg)
    ...
    def update_person(self,state_key):
        while self.pg.get_updated_objects_ids('person', state_key):
            person_updates, last_update = self.pg.get_updated_objects_ids('person', state_key)
            updt_from_pg = self.pg.update_persons(person_updates)
            if person_updates:
                transformed_person_data = self.data_transform.transform_persons_updates(updt_from_pg)
                self.es.load_updates(transformed_person_data, state_key, last_update)
                if person_updates is None:
                    logger.info("Last update is None, but no errors occurred. Please check the data source.")
                else:
